...
# ---- Patched: always create at least one visible sheet; sanitize sheet names; avoid duplicates ----
...
def build_printable_workbook_per_kebele(dfall: pd.DataFrame) -> bytes:
    """Create a per-kebele printable workbook. Never saves an empty workbook."""
    bio = io.BytesIO()

    # If no data, write a fallback sheet
    if dfall is None or not isinstance(dfall, pd.DataFrame) or dfall.empty:
        fallback = pd.DataFrame([
            {"Info": "No sampled households to print.", "Hint": "Run sampling first or check filters."}
        ])
        with pd.ExcelWriter(bio, engine="openpyxl") as writer:
    ...
    with pd.ExcelWriter(bio, engine="openpyxl") as writer:
        # If Kebele is missing or empty → single consolidated sheet
